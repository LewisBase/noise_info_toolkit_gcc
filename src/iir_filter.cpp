...
IIRFilter::IIRFilter(const std::vector<float>& b, const std::vector<float>& a)
    ...
    // Normalize by a[0]
    if (a_[0] != 1.0f) {
        for (float& coeff : b_) coeff /= a_[0];
        for (float& coeff : a_) coeff /= a_[0];
    }
...
void IIRFilter::reset() {
    state_b_.assign(b_.size(), 0.0f);
    state_a_.assign(a_.size(), 0.0f);
}
...
std::vector<float> IIRFilter::process(const std::vector<float>& signal) {
...
std::vector<float> BiquadFilter::process(const std::vector<float>& signal) {
    std::vector<float> result;
    result.reserve(signal.size());
...
IIRCoefficients bilinear_transform(const std::vector<std::complex<double>>& analog_zeros,
    ...
    std::vector<float> b = {static_cast<float>(gain)};
    std::vector<float> a(analog_poles.size() + 1, 0.0f);
    a[0] = 1.0f;
    ...
    if (analog_poles.size() >= 2) {
        b = {static_cast<float>(gain), static_cast<float>(2*gain), static_cast<float>(gain)};  // Simplified numerator
        a = {1.0f,
             static_cast<float>(-2.0 * digital_poles[0].real()),
             static_cast<float>(std::norm(digital_poles[0]))};
    }
...
std::vector<BiquadCoefficients> a_weighting_design(float sample_rate) {
    ...
    float fs = 2.0f / T;  // 2*fs in bilinear transform
    ...
    // Section 1: High-frequency poles at -76617 rad/s (double pole)
    {
        ...
        float a0 = 1.0f + 2.0f*K + K*K;
        float a1 = 2.0f * (K*K - 1.0f);
        float a2 = 1.0f - 2.0f*K + K*K;
    ...
    // Section 2: Mid-frequency poles at -4636 and -676.7 rad/s
    {
        float wc1 = 4636.0f;
        float wc2 = 676.7f;
        float K1 = std::tan(wc1 * T / 2.0f);
        float K2 = std::tan(wc2 * T / 2.0f);

        float a0 = 1.0f + K1 + K2 + K1*K2;
        float a1 = 2.0f * (K1*K2 - 1.0f);
        float a2 = 1.0f - K1 - K2 + K1*K2;
    ...
    for (const auto& coef : sos) {
        std::complex<float> num = std::complex<float>(coef.b0, 0.0f)
                                + std::complex<float>(coef.b1, 0.0f) / z
                                + std::complex<float>(coef.b2, 0.0f) / (z * z);
        std::complex<float> den = std::complex<float>(coef.a0, 0.0f)
                                + std::complex<float>(coef.a1, 0.0f) / z
                                + std::complex<float>(coef.a2, 0.0f) / (z * z);
        H *= num / den;
    }
...
std::vector<BiquadCoefficients> c_weighting_design(float sample_rate) {
    ...
    float current_gain = std::abs(H);
    float norm_factor = 1.0f / current_gain;  // Unity gain at 1 kHz
...
std::vector<BiquadCoefficients> tf2sos(const std::vector<float>& b,
    ...
    if (b.size() <= 3 && a.size() <= 3) {
        ...
        coef.b0 = b[0];
        coef.b1 = (b.size() > 1) ? b[1] : 0.0f;
        coef.b2 = (b.size() > 2) ? b[2] : 0.0f;
        coef.a0 = a[0];
        coef.a1 = (a.size() > 1) ? a[1] : 0.0f;
        coef.a2 = (a.size() > 2) ? a[2] : 0.0f;
        sos.push_back(coef);
    }
...
IIRCoefficients design_octave(double center_freq,
    ...
    float f_lower = center_freq / std::sqrt(2.0f);
    float f_upper = center_freq * std::sqrt(2.0f);
